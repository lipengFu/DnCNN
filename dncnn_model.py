...
class DnCNN(object):
    ...
    def _build_mode(self):
        ...
        conv = slim.conv2d(self._image, 64, [3, 3], activation_fn=tf.nn.relu, weights_regularizer=slim.l2_regularizer(self.hps.weight_decay_rate), scope='conv_1', reuse=self.reuse)

        for i in range(2, self.hps.num_conv):

            conv = slim.conv2d(conv, 64, [3, 3],  activation_fn=None, weights_regularizer=slim.l2_regularizer(self.hps.weight_decay_rate), scope='conv_%d'%(i), reuse=self.reuse)
            conv = slim.batch_norm(conv, scale=True, activation_fn=tf.nn.relu, is_training=self.is_training)

        self.conv = slim.conv2d(conv, 3, [3, 3], activation_fn=None, weights_regularizer=slim.l2_regularizer(self.hps.weight_decay_rate), scope='conv_%d'%(self.hps.num_conv), reuse=self.reuse)
